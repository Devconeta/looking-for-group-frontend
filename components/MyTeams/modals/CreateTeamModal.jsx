...
import { OffChainCreateTeam, OffChainEditTeam } from "../../../pages/api/offChain/post";
...
const CreateTeamModal = ({
  ...
  const { getAppData: refetch, tagOptions, roleOptions } = useContext(OffChainContext);
  ...
  //       const defaultRoles = team.lookingFor.map((role) => {
  ...
  //       const defaultTags = team.tags.map((tag) => {
  ...
  const onCreateTeam = async event => {
    ...
    if (event.target.teamName.value !== "" && event.target.teamMaxMembers.value !== "") {
      if (commpressedFile) {
      ...
      OffChainCreateTeam(teamData)
        .then(response => {
          toastSetter(
            "Party " + response.name + " created sucessfully!, code: " + response.code,
            "success"
          );
          refetch();
        })
      ...
      toastSetter("Team name and members quantity cannot be empty.", "error");
    }
  };
        ...
        <div className="fixed z-30 flex h-full w-full items-center justify-center backdrop-brightness-[30%]">
          <div className="appContainer--myteams !h-[90%] w-[70%] justify-start rounded-xl border-[1px] border-white/30 p-5 text-primaryLight shadow-lg backdrop-blur-lg">
            <div className="mb-2 flex items-center justify-between">
              <h1 className="">🧙 {createTeamModal ? "Create your team" : "Edit your team"}</h1>
              <span
                className="cursor-pointer text-xl text-white"
                onClick={() => {
                  createTeamModal && onCloseTeamCreation();
                  editTeamModal.display && onCloseTeamEdition();
                  setImage("");
                }}
              >
                X
            ...
            <form
              ...
              onSubmit={createTeamModal ? onCreateTeam : onEditTeam}
            >
              <div className="flex flex-row justify-center h-full gap-10">
                <div className="flex flex-col gap-5 w-1/2">
                  <div className="flex h-36 w-full items-center justify-center">
                    ...
                    >
                      <label htmlFor="teamAvatar" className="cursor-pointer flex w-full h-full">
                        {image ? (
                          <img
                            className="rounded-full bg-cover"
                            src={team.avatar != "" ? team.avatar : image}
                          />
                        ) : (
                  ...
                  <div className="flex flex-col gap-1">
                    <label htmlFor="teamSlogan">Slogan:</label>
                    <input
                      className="rounded-sm py-[9px] px-2 text-sm text-black "
                      type="text"
                      name="teamSlogan"
                  ...
                  <div className="flex items-center justify-evenly gap-2 mt-2">
                    <div className="flex items-center gap-2">
                      <label htmlFor="teamMaxMembers">Team size</label>
                      <input
                    ...
                    <div className="flex w-fit items-center gap-1">
                      <label htmlFor="teamVisibility">Make team public?</label>
                      <input
                        className="h-4 w-4"
                        type="checkbox"
                        name="teamVisibility"
                ...
                <div className="flex flex-col w-1/2 h-full  gap-4 2xl:h-auto">
                  <div className="flex flex-col gap-1">
                    <label htmlFor="teamRoles">Roles needed:</label>
                    <Select
                      onChange={options => {
                      ...
                      isMulti
                      isSearchable={false}
                      // defaultValue={defaultRoles.length > 0 && defaultRoles}
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Select
                      ...
                      isMulti
                      isSearchable={false}
                      // defaultValue={defaultTags.length > 0 && defaultTags}
                    />
                  </div>

                  <div className="flex flex-col gap-1">
                    <label htmlFor="teamIdea">Idea:</label>
                    <textarea
                      type="text"
                      name="teamIdea"
                      id="teamIdea"
              ...
              <button className="btn--golden" type="submit">
                {createTeamModal ? "Confirm creation" : "Confirm edition"}
              </button>
            </form>
          </div>
        </div>
