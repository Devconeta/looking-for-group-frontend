...
const PreDeployTreasury = ({ team, setIsPartyDeployed }) => {
  ...
  const preDeployValidation = () => {
    if (deployDisabled) {
      if (!toastifyLimited) {
        toastSetter("The sum of the percentages must be 100%", "warning");
      ...
      Promise.resolve(
        ...
        if (txReceipt === 0) {
          ...
          if (txReceipt.status === 1) {
            toastSetter("Party Treasure created sucessfully!", "success");
      ...
      <div className="flex h-full  gap-4">
        ...
        <div className="w-7/12 2xl:w-8/12 ">
          <div className="flex h-full max-w-[95%] flex-col justify-around 2xl:max-w-[60%] 2xl:text-lg min-h-[200px]">
            <p>
              <span className="italic text-primary">{team.name}</span> party
              treasury is about to be created. The party will receive their
              unique address where the team is gonna receive the prizes.
            </p>

            <div className="flex flex-col gap-1">
              <p>
                We are on the Polygon Mumbai Testnet and working on possible
                bridges and solutions to receive more currencies than Matic, and
                non fungibles in the next iteration.
              </p>
              <p>The deploy is optimized to cost aprox half a cent.</p>
            </div>
          </div>
        </div>
        {/* RIGHT BLOCK BLOCK */}
        <div className="flex w-5/12 flex-col justify-between gap-3 rounded border-[1px] border-white/30 2xl:w-4/12 ">
            ...
            {team.members.map((member, index) => (
              ...
              >
                  ...
                  <Link href={`/user/${member.address}`} className="">
                    <span className="flex cursor-pointer items-center">
                      <img
                        ...
                        src={member.image ? member.image : "user.png"}
                        alt=""
                      />
                ...
                <div className="relative flex">
                  <input
                    onChange={(e) => {
                      onChangePercentage(e.target.value, index);
                    }}
                    ...
                    min="0"
                    className={`w-[3.75rem] ${
                      percentages[index].toString().length == 100
                        ? "pl-[0.4rem]"
                        : ""
          ...
          <button
            ...
            className={`btn--golden ${
              deployDisabled || txState.waitingConfirmation || txState.waitingTx
                ? "disabled"
                : ""
            }`}
