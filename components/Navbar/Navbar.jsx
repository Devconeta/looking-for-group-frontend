...
const Navbar = () => {
  const { userWallet, disconnect, lensMode, setLensMode } = useContext(ContractContext);
  ...
  const handleSwitchLensMode = () => {
    ...
    alert("Lens integration coming soon!");
  };
    ...
    <nav className="w-full flex justify-between items-center h-[5rem] mx-auto z-10 ">
        ...
        <li className="blazing">
          <Link href="/">
            <span className="flex gap-1 items-center cursor-pointer">
              <img className="w-[3.8rem] h-[3.8rem]" src="logo.png" alt="lfg logo" />
              <span>LFG</span>
            </span>
          </Link>
        </li>
      ...
      <div className="flex items-center gap-4">
        {router.pathname !== "/" && (
          <img
            src="lens.jpeg"
            className={`w-9 h-9 rounded-full cursor-pointer ${lensMode ? "" : "opacity-15"}`}
            alt="lens logo"
        ...
        {userWallet.connected ? (
          ...
          <div className="flex flex-col items-end">
            {userWallet.chainId !== 80001 ? (
              <button onClick={openChainModal} className="buttonStandar">
              ...
              <>
                ...
                >
                  <Image
                    ...
                    alt="User Icon"
                  />
                  <p>{userWallet.address && truncAddress(userWallet.address)}</p>
                </button>
                ...
                {toggle && (
                  <div className="flex flex-col items-center justify-center absolute border border-white/30 mt-14 w-24 rounded-sm backdrop-blur-lg text-xl !cursor-pointer z-10">
                    <Link href="/myprofile">
                      <a className="flex p-1 justify-center w-full ">Profile</a>
                    </Link>
