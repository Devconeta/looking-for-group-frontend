...
const TeamCard = ({ team, userData }) => {
  ...
  const orbs = [
    {
      icon: "⚡",
      title: "Porject idea",
      content: team.idea ? team.idea : "The team has not a defined an idea yet",
      match:
  ...
  return (
    <div className="relative mx-5 flex flex-col items-center p-8 2xl:mx-7 ">
      <h3 className="absolute top-[1.4rem] text-xl text-white">{team.name}</h3>

      {/* MEDIA QUERYES */}
      <div className="flex 2xl:hidden">
        <Image className="relative" width="185%" height="185%" src="/orb.png" alt="Team Orb" />
      </div>
      <div className="hidden 2xl:flex">
        <Image className="relative" width="210%" height="210%" src="/orb.png" alt="Team Orb" />
      </div>
      ...
      {orbs.map((orb, index) => (
        <div key={index} className={`absolute top-[50%] right-[0] ${orb.position}`}>
          <Tooltip
            contentColor=""
            color="invert"
            content={
              <div className="flex flex-col">
                <h3 className="text-lg text-primary">{orb.title}</h3>
