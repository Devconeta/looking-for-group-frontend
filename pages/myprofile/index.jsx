...
const mockedData = {
  ...
  tags: ["DeFi", "Web3 Social", "On-chain reputation", "Zero Knowledge Proofs", "Soulbound tokens"],
  level: "Intermediate",
...
const MyProfile = () => {
  const { userData, userTeams, loadingAppData } = useContext(OffChainContext);
    ...
    <AppContainer className="appContainer--myteams">
      {loadingAppData ? (
        ...
        <div className="relative">
          {formattedUserData.lensMode && lensProfile.coverPicture?.original?.url ? (
            <img
          ...
          {formattedUserData.lensMode && lensProfile.picture?.original?.url ? (
            <img
              ...
              <div className="flex items-center gap-4 border-blue-500 ml-2">
                <h3 className="text-xl">
                  {formattedUserData.lensMode && lensProfile.handle
                    ? lensProfile.handle
                    : !formattedUserData.lensMode && formattedUserData.name
                    ? formattedUserData.name + " - " + truncAddress(formattedUserData.address)
                    : "No nickname - " + truncAddress(formattedUserData.address)}
                </h3>
                {/* FOLLOW ON LENS */}
                {formattedUserData.lensMode && (
                  <button className="btn--thin !h-[2rem] !w-[5rem] !bg-primary">Follow</button>
                )}
              </div>

              {formattedUserData.lensMode && (
                <div className="flex">
                  <p className="mx-1">Posts: {lensProfile.stats?.totalPosts}</p>
                  <p className="mx-1">Followers: {lensProfile.stats?.totalFollowers}</p>
                  <p className="mx-1">Following: {lensProfile.stats?.totalFollowing}</p>
                </div>
              )}
            </div>
            {formattedUserData.lensMode && (
              <p className="text-lg italic mb-2">{lensProfile.bio ? lensProfile.bio : ""}</p>
            )}
            ...
            <form className="flex flex-col gap-4 my-4">
                ...
                <span className="flex flex-col mb-2 w-full">
                  ...
                  <p className>
                    {formattedUserData.roles?.length > 0
                      ? formattedUserData.roles.map((item, index) => item + " ")
                      : "No role setted"}
                  </p>
                </span>
                <span className="flex flex-col mb-2 w-full">
                  ...
                  <p className>
                    {formattedUserData.level ? formattedUserData.level : "Expertise not setted"}
                  </p>
                </span>
                <span className="flex flex-col mb-2 w-full">
              ...
              <div className="flex">
                  ...
                  <p className>
                    {formattedUserData.tags?.length > 0
                      ? formattedUserData.tags?.map(
                          (tag, index) =>
                            tag.toString() +
                            (index == formattedUserData.tags.length - 1 ? " " : ", ")
                        )
                      : "No interests setted"}
                  </p>
                </span>
                <span className="flex flex-col mb-2 w-2/3">
                  ...
                  <p className>
                    {formattedUserData.idea ? formattedUserData.idea : "No idea setted"}
                  </p>
                </span>
              </div>
              ...
              <span className=" flex flex-col mb-2 w-full">
                ...
                <p>
                  {formattedUserData.teams?.length > 0
                    ? formattedUserData?.teams?.map((item, index) => item.name)
                    : "No team"}
                </p>
              </span>
              <span className="flex flex-col mb-2">
                <p className="text-lg text-secondary">Social networks</p>
                <p className="">
              ...
              {/* <p>{"member since: " + formattedUserData.createdAt}</p> */}

              {/* {formattedUserData.teams?.map((team, index) => (
                <p>Member of "{team.name}"</p>
              ))} */}
            </form>

            {formattedUserData.lensMode && (
              <form className="w-full flex flex-col">
                <h3 className="text-2xl text-primary mb-5">Share a post on Lens</h3>
                <div className="bg-neutral-900 rounded-lg w-full h-full flex flex-col p-4 gap-4">
                  ...
                  <div className="h-10 flex justify-between">
                    <div className="flex gap-2">
                      <button className="btn--thin !bg-primary/80">Add your team</button>
                      <img src="picture.png" className="orangeFilter w-10 h-10" alt="" />
                    </div>

                    <button className="btn--thin !bg-primary/80">Post</button>
                  </div>
                </div>
              </form>
            )}
